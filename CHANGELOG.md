...
- **Any route can now answer `429` (CAR-126).** The default ceiling — 30 a minute and 500 an hour, per caller per endpoint — was declared when the limiter was written and enforced on nothing: under FastAPI 0.137 SlowAPI's middleware could no longer find the route it was about to limit, so it exempted every request. Only the seven routes carrying an explicit decorator were ever capped. There is nothing new for a client to handle — the reply is the shape the app already renders (`detail` and `retryAfterSeconds`, with `Retry-After`) — but a screen that has never seen a 429 can now see one. The health probes are exempt, and a counter follows the handler rather than the URL, so an id in the path no longer buys a fresh budget.
