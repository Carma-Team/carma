...
- **`PATCH /api/users/me` accepts `driveModeEnabled` / `bluetoothDeviceId` / `bluetoothDeviceName` (issue #7):** the DB columns and read shape (`UserOut`) already existed; the update DTO was missing these fields, so linking a Bluetooth device in the mobile Settings/Bluetooth-picker screens was local-storage-only and lost on reinstall. Now persisted server-side. Client contract: `userApi.updateProfile()`. Enabling drive mode from the app is currently gated to `role=ADMIN` accounts while automatic end-trip detection is stabilized — see `SYSTEM.md` §13.
