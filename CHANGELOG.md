...
- **Delete account, in app:** Settings → Data Management → Delete Account, wired to the existing `DELETE /api/users/me`, with a destructive confirm and a spinner while it runs. Both stores require this for any app with signup; the endpoint existed but nothing called it.
- **`NSBluetoothAlwaysUsageDescription` / `NSBluetoothPeripheralUsageDescription`** in `app.json` — the SDK's `BluetoothManager` would otherwise crash on iOS the moment it touched a Bluetooth API, and App Review rejects a missing purpose string.
