...
- **`driveModeEnabled` is writable on `PATCH /api/users/me` (CAR-153):** automatic trip detection was readable on every profile fetch but nothing could set it, so the driver's choice never left the handset — and since the client merges the server's reply over its cached user, the column's permanent `false` switched the setting back off on the next refresh. Only the keys present in the request are written, so a name change still leaves it alone. The Settings toggle now sends the change (`userApi.updateProfile`) and shows a spinner until it lands; a save the server refuses leaves the setting where it was and raises an error toast. Changing it now needs a connection — it used to "work" offline by writing a value the next refresh discarded, which is the bug itself. The Bluetooth device columns have the same gap and stay as they are on purpose — device selection belongs to the handset.
