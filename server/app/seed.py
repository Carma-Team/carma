...
REWARDS = [
    {"business": "Paz",         "title_he": "50 ש\"ח הנחה בתדלוק", "desc": "50 ש\"ח הנחה בתדלוק בכל תחנות פז", "cat": BusinessCategory.FUEL,          "cost": 500,  "emoji": "⛽"},
    {"business": "Arcaffe",     "title_he": "קפה ומאפה חינם",      "desc": "קפה ומאפה חינם בכל סניפי ארקפה",   "cat": BusinessCategory.FOOD,          "cost": 150,  "emoji": "☕"},
    {"business": "Lime",        "title_he": "15 דקות נסיעה",       "desc": "15 דקות נסיעה בחינם בקורקינט חשמלי","cat": BusinessCategory.ECO,           "cost": 300,  "emoji": "🛴"},
    {"business": "Cinema City", "title_he": "כרטיס VIP לסרט",     "desc": "כרטיס כניסה למתחם ה-VIP",          "cat": BusinessCategory.ENTERTAINMENT, "cost": 1200, "emoji": "🎬"},
    {"business": "Super-Pharm", "title_he": "20% הנחה",           "desc": "קופון הנחה למחלקת הקוסמטיקה",       "cat": BusinessCategory.SHOPPING,      "cost": 400,  "emoji": "🧴"},
]
...
async def run() -> None:
    async with SessionLocal() as db:
        ...
        for biz in BUSINESSES:
            existing_b = await db.scalar(select(Business).where(Business.name == biz["name"]))
            if existing_b is None:
                row = Business(**biz)
                db.add(row)
                biz_by_name[biz["name"]] = row
            else:
                for k, v in biz.items():
                    setattr(existing_b, k, v)
                biz_by_name[biz["name"]] = existing_b
        await db.flush()

        # rewards
        for r in REWARDS:
            biz = biz_by_name[r["business"]]
            existing_r = await db.scalar(
                select(Reward).where(Reward.business_id == biz.id, Reward.title_he == r["title_he"])
            )
            data = dict(
                business_id=biz.id, title_he=r["title_he"], description_he=r["desc"],
                category=r["cat"], cost_points=r["cost"], image_emoji=r["emoji"],
