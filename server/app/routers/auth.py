...
@router.post("/register", response_model=AuthOut, response_model_by_alias=True,
             status_code=status.HTTP_201_CREATED, summary="Register a new user with email+password")
async def register(dto: RegisterIn, db: DbSession) -> AuthOut:
    return await auth_service.register_with_password(db, dto)


@router.post("/login", response_model=AuthOut, response_model_by_alias=True,
...
@router.post("/otp/register", response_model=OtpSent, response_model_by_alias=True,
...
@router.post("/otp/request", response_model=OtpSent, response_model_by_alias=True,
...
@router.post("/otp/verify", response_model=AuthOut, response_model_by_alias=True,
             summary="Exchange an OTP for a JWT")
