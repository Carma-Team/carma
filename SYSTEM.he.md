...
| `Redemption` | מימוש הטבה — QR + status + תוקף 14 יום. | 5.3.1.5 |
...
| `routers/rewards.py` | `services/rewards.py` | רשימת הטבות (פילטר קטגוריה), מימוש (QR base64 רנדומלי, תוקף 14 יום), השוברים שלי. |
...
| POST | `/api/rewards/{id}/redeem` | מימוש — מוריד נקודות, מנפק QR ל-14 יום |
...
| 4.3.6.2 QR חד-פעמי | חד-פעמי; פג ב-14 יום | `consume_voucher` + `VOUCHER_TTL_DAYS = 14` |
...
| **5.2.5 תוקף QR** | 14 יום, לא 5 דק' (ר' להלן) | `Redemption.expires_at` + `VOUCHER_TTL_DAYS` |
...
- **תוקף שובר — 14 יום, ולא 5 הדקות שבאפיון 4.3.6.2 / 5.2.5.** האפיון מאחד שני שעונים שהתעשייה מפרידה ביניהם: כמה זמן הזכאות של הנהג להטבה קיימת, וכמה זמן קוד מוצג מתקבל בסורק. מטרת האבטחה שלו מושגת על ידי `consume_voucher`, שמוציא שובר מ-`PENDING` פעם אחת בלבד — קוד ששותף או צולם חסר ערך אחרי שמומש, ולא משנה מה כתוב בתוקף. הצמדת השעון הקצר לזכאות עלתה לנהגים בנקודות ששילמו: אין החזר על שובר שפג (CAR-24). 14 יום ולא יותר, כי שובר שלא מומש תופס יחידה מהמלאי של העסק לכל אורך חייו.
