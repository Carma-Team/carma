...
- **מצב נסיעה אוטומטי (BT auto start/stop) מוגבל כרגע למנהלי מערכת:** המנגנון (`User.drive_mode_enabled` + `bluetooth_device_id`/`bluetooth_device_name`, נשמר דרך `PATCH /api/users/me`) קיים מקצה לקצה, אך סיום נסיעה אוטומטי בעת ניתוק BT עדיין בתהליך ייצוב. עד לאימות אמינותו, הפעלתו במסך ההגדרות במובייל מוגבלת למשתמשי `role=ADMIN` בלבד — שאר המשתמשים רואים הודעת התנצלות והתכונה נשארת כבויה. התחלה/סיום ידניים ואוטומטיים עובדים ללא תלות בסיווג; רק המסלול המופעל ע"י BT חסום. ראו issue #7.
