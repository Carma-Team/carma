...
| `Redemption` | Redemption of a reward — QR + status + 14-day validity. | 5.3.1.5 |
...
| `routers/rewards.py` | `services/rewards.py` | List rewards (filter by category), redeem (random base64 QR, 14-day validity), my vouchers. |
...
| POST | `/api/rewards/{id}/redeem` | Redeem — debits points, issues a 14-day QR |
...
| 4.3.6.2 One-time QR | Single use; expires in 14 days | `consume_voucher` + `VOUCHER_TTL_DAYS = 14` |
...
| **5.2.5 QR validity** | Expiry — 14 days, not 5 min (see below) | `Redemption.expires_at` + `VOUCHER_TTL_DAYS` |
