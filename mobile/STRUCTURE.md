...
| `tripSummary.ts` | May | One shape for the end-of-trip summary, built either from the device's own trip data or from a trip the server returned. Both summary surfaces render this shape, so neither can show a field the other does not. It also owns the rule for which unscored state a row is in, which the trip list reads too so the list and the detail screen cannot disagree. |
...
| `vehicleKey.ts` | May | The trip's vehicle key: an HMAC of the connected car's Bluetooth address, truncated to the 32 hex characters the payload field holds. The one place a raw MAC address is read, so it is also the one place it can leak from; returns null rather than a key derived from an unconfigured salt. |
| `tripPayload.ts` | May | The body a finished trip is saved with: the canonical raw-sensor digest, its signature, and the mapping from what the SDK measured onto what the contract carries. Sends the score fields as zeros because the contract has them and the server is the sole oracle, and lets an unsigned payload through rather than losing the trip to a signing failure. |
