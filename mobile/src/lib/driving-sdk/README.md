...
| `sensors/SensorManager.ts` | Owns the GPS and IMU subscriptions and their lifecycle, accumulates distance and speed from the location stream, and reports on every fix what the sensors are actually delivering. Detection is next door; this file decides whether the IMU counts as live and hands the samples over. |
| `sensors/motionEvents.ts` | The detection itself: an accelerometer stream accumulates a peak and a mean, a GPS fix closes the window and decides whether it was a brake, an acceleration or a turn, and the vehicle-frame estimate is taught from the pair. Told whether the IMU is live rather than deciding it. |
